...
public class LotacaoController {
    ...
    // @GetMapping
    // public CollectionModel<EntityModel<ObterLotacaoDTO>> getAll() {
    //     List<EntityModel<ObterLotacaoDTO>> lotacoes = repository.findAll().stream()
    //             .map(lotacaoMapper::toObterLotacaoDTO)
    //             .map(assembler::toModel)
    //             .collect(Collectors.toList());

    //     return CollectionModel.of(lotacoes, linkTo(methodOn(LotacaoController.class).getAll()).withSelfRel());
    // }
    ...
    // @PostMapping
    // public Lotacao create(@RequestBody CriarLotacaoDTO criarLotacaoDto) {
